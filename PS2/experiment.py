...
def part_1a():
    ...
    for img_in, label in zip(input_images, output_labels):
        ...
        tl = cv2.imread("{}/input_images/{}.png".format(ROOT_DIR, img_in))
        coords, state = ps2.traffic_light_detection(tl, radii_range)

        img_out = draw_tl_center(tl, coords, state)
        cv2.imwrite("{}/output/{}.png".format(ROOT_DIR, label), img_out)
...
def part_1b():
    ...
    for img_in, label, fn, name in zip(input_images, output_labels, sign_fns,
        ...
        sign_img = cv2.imread("{}/input_images/{}.png".format(ROOT_DIR, img_in))
        coords = fn(sign_img)

        temp_dict = {name: coords}
        img_out = mark_traffic_signs(sign_img, temp_dict)
        cv2.imwrite("{}/output/{}.jpg".format(ROOT_DIR, label), img_out)
...
def template_match_test():
    ...
    templates = [
        'tl_template', 'construction_template', 'waldo1_template'
    ]
    for img_fl, img_template_fl, label in zip(
        ...
        img_in = cv2.imread("{}/input_images/{}.png".format(ROOT_DIR, img_fl))
        img_template = cv2.imread("{}/input_images/{}.png".format(ROOT_DIR, img_template_fl))
        ...
        for method in ("tm_ssd", "tm_nssd", "tm_ccor", "tm_nccor"):
            ...
            cv2.imwrite("{}/output/{}-{}.jpg".format(ROOT_DIR, method, label), im_out)
...
def compression_runner():
    ...
    cv2.imwrite(OUTPUT_DIR + 'ps2-4-a-3-freq.jpg', np.abs(compressed_frequency_img))
...
def low_pass_filter_runner():
    ...
    cv2.imwrite(OUTPUT_DIR + 'ps2-5-a-3-freq.jpg', np.abs(low_pass_frequency_img_mag))
