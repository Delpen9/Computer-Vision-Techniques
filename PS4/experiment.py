...
def scale_u_and_v(u, v, level, pyr):
    ...
    while True:
        if u.shape[0] < pyr[0].shape[0]:
            u = ps4.expand_image(u)
            v = ps4.expand_image(v)
        elif u.shape[0] > pyr[0].shape[1]:
            u = ps4.reduce_image(u)
            v = ps4.reduce_image(v)
        else:
            break
    return u, v
...
def part_1a():
    ...
    sigma = 0.01  # TODO: Select a sigma value if you are using a gaussian kernel
...
def part_1b():
    ...
    k_size = 50  # TODO: Select a kernel size
    k_type = "gaussian"  # TODO: Select a kernel type
    sigma = 0.0000001 # TODO: Select a sigma value if you are using a gaussian kernel
...
def part_2():
    ...
    # cv2.imwrite(os.spath.join(output_dir, "ps4-2-a-1.png"), yos_img_01_g_pyr_img)
