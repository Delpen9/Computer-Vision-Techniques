...
# I/O directories
input_dir_part_1 = fr'{ROOT_DIR}/input_images_part1'
input_dir_part_2 = fr'{ROOT_DIR}/input_images_part2'
output_dir = fr'{ROOT_DIR}/output'

YALE_FACES_DIR = os.path.join(input_dir_part_2, 'Yalefaces')
FRUITS_DIR = os.path.join(input_dir_part_1, 'fruits')
POS_DIR = os.path.join(input_dir_part_2, "pos")
POS2_DIR = os.path.join(input_dir_part_2, "pos2")
NEG_DIR = os.path.join(input_dir_part_1, "neg")
NEG2_DIR = os.path.join(input_dir_part_2, "neg2")
...
def visualize_mean_face(x_mean, size, new_dims):
    ...
    """
    mean_face_2d_array = np.reshape(x_mean, (size))
    mean_face_2d_array = mean_face_2d_array / np.sum(mean_face_2d_array)
    mean_face_2d_array = cv2.resize(mean_face_2d_array, (new_dims), cv2.INTER_AREA) * 255

    return mean_face_2d_array
...
def part_1a_1b():
    ...
    x_mean_image = x_mean_image * 255

    cv2.imshow('mean_image', x_mean_image)
    cv2.waitKey(0)
...
def part_2a():
    ...
    idx_y0 = np.where(y == y0)
    idx_y1 = np.where(y == y1)
    idx = np.append(idx_y0, idx_y1)

    X = X[idx, :]
    y = y[idx]

    # Label them 1 and -1
    y0_ids = np.where(y == y0)
    y1_ids = np.where(y == y1)
    ...
    acc_list = rand_y - ytrain
    rand_accuracy = len(np.where(acc_list == 0)[0]) / len(ytrain) * 100
    ...
    wk_clf.train()
    wk_results = np.array([wk_clf.predict(x) for x in Xtrain])

    # TODO: find which of these labels match ytrain and report its accuracy
    acc_list = wk_results - ytrain
    wk_accuracy = len(np.where(acc_list == 0)[0]) / len(ytrain) * 100
    ...
    acc_list = rand_y - ytest
    rand_accuracy = len(np.where(acc_list == 0)[0]) / len(ytest) * 100
    print('(Random) Testing accuracy: {0:.2f}%'.format(rand_accuracy))

    # Using Weak Classifier
    wk_results = np.array([wk_clf.predict(x) for x in Xtest])

    # TODO: find which of these labels match ytest and report its accuracy
    acc_list = wk_results - ytest
    wk_accuracy = len(np.where(acc_list == 0)[0]) / len(ytest) * 100
...
def part_3a():
    ...
    """
    • Input: type = two_horizontal (2, 1); position = (25, 30); size = (50, 100) Output: ps6-3-a-1.png
    • Input: type = two_vertical (1, 2); position = (10, 25); size = (50, 150) Output: ps6-3-a-2.png
    • Input: type = three_horizontal (3, 1); position = (50, 50); size = (100, 50) Output: ps6-3-a-3.png
    • Input: type = three_vertical (1, 3); position = (50, 125); size = (100, 50) Output: ps6-3-a-4.png
    • Input: type = four_square (2, 2); position = (50, 25); size = (100, 150) Output: ps6-3-a-5.png
    """
